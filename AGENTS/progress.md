...
**Last session summary (v1.7.0):** Added **Remote Lists** — selector values pulled from published plain-text URLs (e.g. `https://core.telegram.org/resources/cidr.txt`). New `internal/remotelist` package owns list CRUD, per-list scheduling and change detection; `internal/routing/remote_lists.go` owns the selector semantics (kind → selector mapping, per-kind normalization, expansion). A list is `name + url + kind (cidr|asn|domain|wildcard) + refresh interval + enabled`; rules reference it by name from a new **Remote Lists** selector box and the entries are merged into the rule's destination CIDR/ASN/domain/wildcard selectors at apply time only — `ListGroups` returns expanded groups (routing, resolver, prewarm, inspectors), `ListGroupsRaw` returns them unexpanded (group editor API + backup export), so the editor and backups only ever hold the reference. Refresh is conditional (ETag/If-Modified-Since) and the parsed entry set is SHA-256 fingerprinted, so an unchanged or merely reformatted source causes **no** routing side effects; a real change fires one `routing.Apply()` (ipsets + dnsmasq + iptables) plus a resolver run for asn/domain/wildcard kinds. Safety properties: an empty/unfetched/disabled list still yields an empty destination ipset so the rule matches nothing instead of diverting all of its source traffic; a failed fetch keeps the previously cached entries; renaming or deleting a referenced list returns 409. Contents are cached in-process and invalidated on mutation, because `ListGroups` runs on every 2s stats broadcast. Bounds: 8 MiB body; per-kind entry caps (65536 cidr/asn, 5000 domain, 250 wildcard — domain/wildcard entries each cost a resolver selector and a pre-warm query per interface); interval 5 min–30 days (default 6 h), 15-min retry window for failing lists. Code review (high) surfaced and fixed: a 200 response parsing to zero usable entries silently wiped a live list (now a recorded failure that keeps cached entries); case-only duplicate names collapsed two lists into one contents entry (name is now `COLLATE NOCASE UNIQUE`); a cache load/store race could republish a stale snapshot (generation counter); "Refresh All" fetched disabled lists' URLs; `MarkUnchanged` left a stale skipped count; `SaveEntries` did one exec per row under the single SQLite write connection (prepared statement); backup restore now warns about rule references to lists the snapshot does not carry. Backup snapshot now carries list definitions plus per-rule references. Validated end-to-end against the live Telegram list (45 entries → 5 v4 + 3 v6 aggregated ipset entries) and the UI verified with Playwright. Released as v1.7.0.

**Prior session summary (v1.6.0):** Hardened speed-test server selection and added a second provider. Root-caused a user-reported "SGP tunnel only 15 Mbps": on-device testing showed the tunnel is fine (raw curl ~186 Mbps, engine now 245–520 Mbps) — the 15 Mbps was a transient throttled Ookla server pick. Fixed by making Ookla selection latency-filter the nearest 5 candidates, then run a short download probe on the top 3 and pick the fastest (`probeDownloadThroughput`), instead of blindly taking lowest latency. Added a **Netflix fast.com** provider (`internal/speedtest/fast.go`): scrapes the session token, calls the v2 targets API, and drives download/upload/latency across multiple OCA URLs (range inserted into the path before the query). Refactored the engine around a provider-agnostic `target` interface so both backends share the transfer/sampling machinery; extracted shared latency helpers (`httpRTT`, `minAndJitter`). Added a provider selector (Ookla / fast.com) to the modal that re-runs on switch, and a `provider` query param to the SSE endpoint. Replaced the single-run guard's fail-fast CAS with a bounded-wait `acquireSpeedtest` so switching provider (close+reopen stream) doesn't race the previous run's async guard release. Validated on-device: WAN 2157/1081 (Ookla) & 724/827 (fast); SGP tunnel 245/218 (Ookla) & 276/217 (fast), all geolocating through the tunnel. Released as v1.6.0.

**Prior session summary (v1.5.0):** Added per-interface speed tests (Ookla/speedtest.net protocol) to every dashboard interface card, WAN and VPN. A speedometer button next to each card's flow-inspector loupe opens a modal that streams live results over SSE (`GET /api/speedtest/stream?iface=<device>&label=<label>`): server selection, ping/jitter, and live download/upload with a running Chart.js graph. Implemented a focused pure-Go Ookla client in `internal/speedtest/` (server-list fetch → canonical-host resolution via the sponsor's HTTP→HTTPS redirect → `random{n}x{n}.jpg` download and `upload.php` POST with atomic byte-counting and duration-bounded sampling). Each test binds egress to the target device via `SO_BINDTODEVICE`, so a VPN test genuinely traverses the tunnel and geolocates to its exit; extracted that bind helper into a shared `internal/netbind` package (prewarm now uses it too). A single-run guard (`Server.speedtestActive`) prevents concurrent tests from corrupting each other's numbers, and the request context cancels the test when the browser disconnects. Validated on the live UDM: WAN 2085/1060 Mbps @ 2.7 ms; the DE tunnel auto-picked a Frankfurt server at 107 ms / ~477 Mbps down. Released as v1.5.0.

**Prior session summary (v1.4.1):** Field hotfix for the v1.4.0 pre-warm overhaul. Live diagnosis on the UDM showed runs crawling because the user's 3 ISP nameservers are unreachable over the VPNs — every query timed out and, with attempts=3, cost 30s, re-issued for all 123 domains (hours). Added a per-run per-resolver circuit breaker (`resolverGate`): after `defaultResolverFailureThreshold` (10) consecutive failed queries a resolver is disabled for the rest of the run and skipped before acquiring a query slot; on any success the counter resets. A resolver that has already failed drops to a single attempt (no more 3× waste on a dead resolver) while healthy resolvers keep the full retry budget. Added always-visible logging to the main `split-vpn-webui.log` (stdlib `log.Printf`) for run start, run finish/failed/canceled summaries, and resolver-disabled events — no need to enable debug diagnostics. Released as v1.4.1.

**Prior session summary (v1.4.0):** Overhauled the DNS pre-warmer for resilience: every DNS query is now bounded by a per-request timeout and retried up to `PrewarmQueryAttempts` times (default 3) before logging and moving on; a run-wide semaphore now caps concurrent in-flight queries and fans out each task's CNAME/A/AAAA queries so no single wildcard domain can stall the run for tens of minutes. crt.sh wildcard discovery stays single-shot (time-bounded, no retry) to avoid amplifying its rate limits. Added persistent `prewarmQueryAttempts` setting and surfaced Parallelism + Attempts in the DNS Pre-Warm card. Added a Settings → Service "Restart split-vpn-webui" button (`POST /api/system/restart`, systemd-guarded) to force-restart a stuck process. Released as v1.4.0.
