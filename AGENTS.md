...
* Work on the `main` branch by default and commit work as appropriate, unless explicitly directed to use a different branch.
...
- Its own systemd unit files (e.g. `wg0-sgp.contoso.com.service`, `wg-quick@*.service`).
...
**OpenVPN:**
- Config file format: standard `.ovpn` client config. See `/Users/maciekish/Developer/Repositories/Appulize/unifi-split-vpn/web.contoso.com/DreamMachine.ovpn` for a real example.
...
- `sgp.contoso.com/wg0.conf` — real WireGuard client config (use as format reference)
- `web.contoso.com/DreamMachine.ovpn` — real OpenVPN client config (use as format reference)
- `sgp.contoso.com/vpn.conf` and `web.contoso.com/vpn.conf` — real split-VPN routing metadata files
